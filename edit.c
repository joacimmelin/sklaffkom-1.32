		...
		*space = '\0';
		...
		if ((strstr(MSG_EDREAD, cmd) && (strlen(cmd) > EDREADN))
			...
			while (tb) {
			    ...
			    else c2 = MSG_YESANSWER;
			    if ((c2 > 0x40) && (c2 < 0x5e)) c2 = c2 + 0x20;
			    output("\n");
			    if (c2 == 'q') break;
			    if (c2 == 'a') {
				qt = 1;
		  ...
		  cmd_my(upcasearg);
		    ...
		    output(MSG_EDHELP4);
		    output(MSG_EDHELP5);
		    if (allow_say) output(MSG_EDHELP6);
		    if (allow_say) output(MSG_EDHELP7);
		    if (edit_text) {
			output(MSG_EDHELP8);
			output(MSG_EDHELP9);
	    ...
	    ptr->next = NULL;
	    ptr->line[0] = '\0';
	    if (waste[0]) strcpy(ptr->line, waste);
	    waste[0] = '\0';
	}
    }
...
int resume_aborted_edit()
{
    ...
    strcpy(fname, Home);
    strcat(fname, DEAD_FILE);
    if((fd = open_file(fname, OPEN_QUIET)) == -1) return 1;
