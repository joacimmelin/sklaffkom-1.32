...
int main(int argc, char *argv[])
{
    ...
    if ((fd = open_file(CONF_FILE, 0)) == -1) exit(1);
    ...
    while ((strcmp(argv[1], ce.name) != 0) && buf)
	    buf = get_conf_entry(buf, &ce);
    free (oldbuf);
    if (strcmp(argv[1], ce.name) == 0) confid = ce.num;
    else exit(0);
	...
	if ((fd = open(article, O_RDONLY, 0)) != -1) {
		...
		if (stat(article, &st) != -1) mtime = st.st_mtime;
		...
		while (run <= ce.last_text) {
			...
			if ((buf2 = read_file(fd2)) == NULL) exit(1);
			oldbuf2 = buf2;
			if (close(fd2) == -1) exit(1);
...
long send_news(int confid, char *mbuf, int ouid, int ogrp, long com)
{
    ...
    oldbuf = buf;
    while ((buf = get_conf_entry(buf, &ce))) if (ce.num == confid) break;
    if (ce.num == confid) {
	ce.last_text++;
...
long find_ref(char *refer)
{
    ...
    look = top;
    while (look) {
	if (!strcmp(look->id, refer)) return look->num;
	look = look->next;
    }
    return 0;
}
